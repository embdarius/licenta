...
| [`docs/agents/case-generation-agent.md`](docs/agents/case-generation-agent.md) | Case Generation Agent (Phase 4, offline/benchmark-only): tabular row → grounded NL patient case, and the 3-way end-to-end vs tabular benchmark |
...
- **Phase 4 — Text Generation / Case Generation Agent:** In progress. The Case Generation Agent (`src/proiect_licenta/case_generation.py`, `uv run generate_cases`) translates MIMIC-IV tabular rows into grounded natural-language patient cases (LLM phrases the narrative; everything else is deterministic from the row; a validator enforces no invented facts). `benchmarks/benchmark_pipeline_e2e.py` runs a stratified 20-case set end-to-end through the full crew and compares accuracy against the tool-direct and feature-vector tabular baselines on the same stay_ids. Offline/benchmark-only — not in the live patient crew. **First 20-case run** (13 admit / 7 discharge, 0 grounding-flagged): acuity 70% tool-direct ≡ feature-vector vs 60% E2E (the clean NL-translation signal, −10pp); diagnosis/department are dominated by the disposition gate (the gated tool/E2E paths diagnose only 6–7/13 admitted cases vs the unconditional feature-vector's 13/13). NL-fidelity: gender/arrival 20/20, age 18/20, complaint token Jaccard 0.66. The run also surfaced that the runtime disposition tool under-admits vs its own training features (snapshot-only longitudinal fallback). 20 cases is a noisy validation scale — scaling up is the next step. See [`docs/agents/case-generation-agent.md`](docs/agents/case-generation-agent.md).
