...
| [`docs/results/rebenchmark_v3/`](docs/results/rebenchmark_v3/) | **Detailed v3 re-benchmark audit (2026-06-28):** regression PASS vs `ceiling.json`, full per-class/calibration metrics, disposition per-threshold operating points, graded ICD near-miss, and the generated-cases Flash-vs-MedGemma head-to-head + NLP-parser-cost decomposition. Raw artifacts in `benchmarks/audit/2026-06-28/` |
...
| [`README.md`](README.md) | Interactive web interface — the **live conversational runtime** (drives the real agentic crew over SSE: per-session crew thread, injected interactive-tool channel, event-bus telemetry, intake-confirmation gate, rich result cards) + the retained non-interactive stage/parity test path (merged into the root README) |
...
| **Triage v3 iter 2 — Acuity (5 classes, kept)** | **67.55%** | — | **98.2% within-1-level; + sections 1.1 (longer training) + 1.2 (ordinal-aware weights + QWK early stopping). −0.46pp headline accuracy vs v2 but −2.11pp under-triage rate and ESI 5 recall recovered to 26.82% (+5pp vs v2). Clinical-safety trade kept over iter 1's higher headline accuracy. A constrained Optuna sweep of the inherited Group-2 regularization hyperparameters (2026-06-26) found no config improving QWK beyond noise while holding under-triage ≤ incumbent — confirming these values are near-optimal. See [`docs/results/triage_hpo/`](docs/results/triage_hpo/).** |
| Triage v1 — Disposition (2 classes) | 75.9% | — | ROC AUC 0.84 (no vitals) |
| Triage v2 — Disposition (2 classes) | 76.2% | — | ROC AUC 0.84 |
| **Triage v3 iter 2 — Disposition (2 classes, kept)** | **78.0%** | — | **ROC AUC 0.86. +1.80pp vs v2; `pmh_Blood and Blood-Forming Organs` is rank 5 in the disposition model — PMH is materially driving the admit/discharge signal. Iter 1 and iter 2 are essentially identical on disposition (77.96% → 77.98%). The 2026-06-26 Group-2 sweep found the inherited config to be the outright best (ROC AUC 0.8641 inner-val).** |
...
| **Doctor Stage-2 exact-ICD (3-char rollup, on v3 nurse)** | **—** | **—** | **Retrieval cascade (TF-IDF prototype cosine + prevalence, α=0.60). Within the top-5 predicted categories: 62.9% of admitted patients get their exact 3-char ICD in the top-5-cats×top-5-codes set, 55.5% in a flat top-10; oracle-category (true category given) 67.0%@5 / 79.9%@10 — note this is the **text+prevalence blend only**; fully stacked with the vitals + gated-PMH terms the oracle@5 rises to **69.8% overall / 72.7% on the has-PMH subset** (full-code 49.2%→53.7% / 58.3%). The 67.0% / 49.2% headlines are the pre-vitals/pre-PMH blend; the per-term increments are itemized below. Re-verified 2026-06-17. Blend beats prevalence-only (+11.8pp@5) and cosine-only (+16.7pp@5). Full-code secondary: 49.2%@5 oracle (blend). Advisory only — does not change category/department. Shipped 2026-06-11. v3_base (pre-nurse) vs v3-nurse: nurse data adds +1.0pp E2E union / +1.3pp flat-10 (entirely via Stage-1 category recall 90.6%→92.8%); the Stage-2 oracle ceiling is model-agnostic and identical for both. Vitals-conditioned 3rd term (28-dim z-scored physiology: snapshot vitals+flags + cardiac rhythm + vital deltas, weights 0.3/0.4/0.3) lifts the oracle ceiling +2.15pp rollup / +3.34pp full-code @5 and E2E union +2.0/+3.1pp. Gated PMH 4th term (13 prior-dx flags + same-complaint, applied only when the patient has history) adds +0.97pp rollup / +1.77pp full-code @5 on the 65% has-PMH subset (no-PMH provably unchanged). All wired into the runtime tool.** |
...
> **Re-benchmark audit (2026-06-28):** every headline number above was re-verified at full detail — all **29 reproduce `ceiling.json` within <1e-4** (regression PASS). The audit adds per-class/confusion/calibration tables, a disposition **per-threshold operating-point** table ({0.15/0.20/0.30/0.40-live/0.50} + 0.10–0.90 sweep), graded ICD near-miss metrics (TF-IDF / Gemini / ICD-tree), and the generated-cases **Flash-vs-MedGemma** head-to-head with an NLP-parser-cost decomposition. Three caveats surfaced for the thesis: the department nurse +10.1pp is partly majority-class concentration (balanced-acc −4.4); ICD strict@5 understates clinical value (lead with graded — rollup oracle@5 graded 0.92 Gemini / 0.78 tree vs strict 0.69); and the disposition lift over the triage cascade is AUC/Brier/calibration, not the fixed-threshold under/over. Full write-up + numbers: [`docs/results/rebenchmark_v3/`](docs/results/rebenchmark_v3/); raw artifacts in `benchmarks/audit/2026-06-28/`.
...
11. **Stage-2 exact-ICD resolution (Doctor v3, 2026-06-11)** — A retrieval cascade on top of the v3 diagnosis model predicts the *exact* ICD diagnosis within each predicted category (3-char rollup headline + full-code secondary). Candidates are ranked by a 3-term blend `w_text·text_cos + w_vit·vitals + w_prev·prevalence` (each min-max'd within category): text cosine compares the complaint's TF-IDF vector to per-code prototype centroids; the **vitals term** compares the patient's 28-dim z-scored physiology (snapshot vitals + abnormal flags + cardiac rhythm one-hots + per-vital trajectory deltas) to per-code vitals centroids; the **PMH term** (gated — applied only to patients with prior history) compares the patient's prior-diagnosis flags + same-complaint signal to per-code PMH centroids built from has-history rows; prevalence is the within-category base rate. Weights grid-searched offline (3-term 0.3/0.4/0.3 for no-history; gated 4-term 0.2/0.4/0.2/0.2 for has-history). The **vitals term** is a 28-dim z-scored physiology vector (snapshot vitals + flags + cardiac rhythm + per-vital deltas) and lifts the Stage-2 **oracle ceiling** +2.15pp rollup / +3.34pp full-code @5 (out-of-sample); fully stacked (text+vitals+gated-PMH+prevalence) the oracle@5 reaches **69.8% rollup overall / 72.7% on the has-PMH subset** (vs 67.0% for the text+prevalence blend; re-verified 2026-06-17). It does **not** retrain or alter the category/department predictions — it's advisory, surfaced as `diagnosis_prediction.exact_diagnoses` and gracefully skipped if the artifact isn't built. No new dependencies (reuses the TF-IDF vectorizer + snapshot vitals). See [`docs/agents/doctor-agent.md#stage-2--exact-icd-resolution-within-categories-shipped-2026-06-11`](docs/agents/doctor-agent.md#stage-2--exact-icd-resolution-within-categories-shipped-2026-06-11).
...
**Experiment A scaled to 150 cases (2026-06-17).** Re-ran the same-mode comparison on **150
cases (98 admitted / 52 discharged)** with the **lookup-enabled** `parser_llm` bypass (prior-
encounter PMH + reconciled meds fed to the tools, matching `feature_vector`'s PMH source), so the
only feature differing from the tabular path is the LLM-parsed complaint. Methodology check passed
(4 LLM-free modes byte-identical; MedGemma 0 parse errors). **MedGemma-4B shows a small,
directionally consistent edge on every clinical-content target** — diagnosis@1 +3.1pp (.398 vs
.367), @3 +4.1pp, department@1 +1.0pp, disposition +1.3pp, ICD@5/union +2.1pp — **despite a
marginally lower complaint Jaccard** (.413 vs .425), i.e. it paraphrases into more clinically-
aligned terminology. **Flash wins ESI acuity** (+4.0pp). Deltas are 1–4 cases (within per-metric
CI) but consistent in direction — a meaningful sharpening of the n=20 tie, framed as a modest
medical-domain edge, not a decisive win. With equal features + equal gating, both parsers lose
~13pp (Flash) / ~10pp (MedGemma) on diag@1 vs the gated exact-complaint ceiling — the irreducible
clinical-shorthand→lay-language paraphrase cost (e.g. "stomach pain"→`Abd pain`, "brain bleed"→
`SDH`), shrinkable on the system side (clinical-term prompt / synonym map / paraphrase-robust
embeddings) but **not** by clinicalizing the benchmark input. Results saved to
`artifacts/benchmarks/comparison_n150.json` (+ raw `parser_{flash,medgemma}_n150.json`); the
LLM-independent tabular ceiling (re-verified, regression-gated vs the headline table) is in
`artifacts/benchmarks/tabular/ceiling.json`. Full writeup + per-target decomposition:
[`docs/llm-backend.md` §5b](docs/llm-backend.md). **Operational caveat** unchanged: Flash drives
the full agentic crew; MedGemma-4B cannot out of the box, so the `parser-llm` bypass is its
evaluation path. **Next:** Experiment B (case-generation grounding quality, needs a live MedGemma
tunnel) — see [`docs/future-work.md`](docs/future-work.md) and [`docs/llm-backend.md` §7](docs/llm-backend.md).
...
- **Hyperparameter search (Optuna, reporting-only):** Two reporting-only sweeps per agent, neither redeploys the live models. **Group-2** (the 8 inherited XGBoost regularization knobs) is DONE for triage (acuity + disposition) and doctor (department + disposition) — the inherited box is near-optimal ([`docs/results/triage_hpo/`](docs/results/triage_hpo/), [`docs/results/doctor_hpo/`](docs/results/doctor_hpo/)). **Group-1** (the clinical-safety / cost config the Group-2 sweeps froze) is now IMPLEMENTED as a sibling study in the same engines + notebooks (`--group group1`, separate `*_g1` files): **triage is multi-objective Pareto** (NSGA-II over the `ESI_EXTREME_BOOST` vector + lr/n_estimators + disposition threshold/`scale_pos_weight`, objectives exact-acc / −under-triage / ESI-5 recall), **doctor is single-objective** (department macro-F1, disposition constrained AUC, over `class_weight_exponent`/`scale_pos_weight_exponent` + lr/n_estimators). A confirmed Group-1 best can be chained into a Group-2 re-run (`--use-group1-best`). **Doctor Group-1 RUN (2026-06-27, 10 trials/head): near-optimal, not redeployed** — both heads confirm the live config (best department `cw=0.78` fails out-of-sample; disposition selected `spw≈0.504` ≈ live sqrt; lr/n_est plateau). The lone non-trivial mover across the whole doctor program is the Group-2 disposition trial 7 (−1.15pp under-triage), kept documented-only / optional-future. **Triage Group-1 RUN (2026-06-27, 10 trials/head): the strongest keep-live result** — the multi-objective search re-selected the incumbent on both heads and no frontier config dominates it; acuity incumbent has the highest ESI-5 recall on the frontier (boost 1.5/1.3/2.0 validated), disposition incumbent has the best ROC AUC (the rest of that frontier is just threshold movement). Not redeployed; no redeploy candidate exists for triage. See [`docs/results/triage_hpo_group1/`](docs/results/triage_hpo_group1/), [`docs/results/doctor_hpo_group1/`](docs/results/doctor_hpo_group1/).
- **Phase 4 — Text Generation / Case Generation Agent:** In progress. The Case Generation Agent (`src/proiect_licenta/case_generation.py`, `uv run generate_cases`) translates MIMIC-IV tabular rows into grounded natural-language patient cases (LLM phrases the narrative; everything else is deterministic from the row; a validator enforces no invented facts). `benchmarks/benchmark_pipeline_e2e.py` runs a stratified 20-case set end-to-end through the full crew and compares accuracy against three tabular baselines on the same stay_ids — **tool-direct**, **feature-vector** (unconditional, the existing benchmark methodology), and **feature-vector-gated** (gated by the disposition model's own verdict). The four columns cleanly separate the three losses: disposition-gate, runtime feature-degradation, and LLM/NLP. Offline/benchmark-only — not in the live patient crew. **20-case run** (13 admit / 7 discharge, 0 grounding-flagged, disposition threshold 0.40): acuity ties at 70% across the tabular columns (E2E carries LLM run-to-run jitter). The benchmark surfaced two fixes that closed most of the tool-direct↔feature-vector gap: (1) the **multi-reading-vitals fix** — the runtime under-admitted because it rebuilt longitudinal vitals from a single snapshot, so the nurse now collects an optional 2nd reading → shared `build_longitudinal_block` → new `vital_trajectory_json` arg on the disposition + v3 tools (snapshot fallback preserved); (2) **disposition threshold 0.50→0.40** (max F1/Youden on the 83K test split). Combined, tool-direct disposition rose 65%→**90%** and diagnosis@1 23%→**61.5%** (Dx coverage 6/13→**11/13**, vs the gated ceiling 12/13). The residual gap to the *ungated* feature-vector is the disposition gate itself (by design); next levers are a stronger disposition model and scaling beyond 20 cases. **Stage-2 exact-ICD added to this benchmark (2026-06-15, doctor+nurse):** ICD `union` recall is mode-invariant at **69.2% incl. live E2E** (matches `benchmark_icd_resolution.py`), confirming the resolver survives the full free-text pipeline; the flat-list metrics (`@1`/`@5`/`flat@10`) wobble ±1–2 cases by category-softmax shape (a peakier softmax can *lower* flat exact-code recall), and `@1` is expectedly weak under paraphrase (7.7% E2E). Base-doctor (v3_base) live-E2E exact-ICD remains deferred (its tool doesn't wire the resolver). See [`docs/agents/case-generation-agent.md`](docs/agents/case-generation-agent.md).
- **Web interface — live conversational runtime (shipped):** An interactive website (`webapp/`, FastAPI + React/Vite/Tailwind) that **drives the real agentic crew** (the same `ProiectLicenta().crew().kickoff(...)` as `uv run run_crew`) and renders it as a live clinical conversation — showing which agent is active, the questions they ask (answered in-browser), and the tools running in real time, with structured tool outputs surfaced inline as rich result cards and a per-task collapsible reasoning panel. The terminal's blocking `input()` is bridged to the browser via a per-session **channel** injected into the interactive tools (`src/proiect_licenta/interaction.py`; stdin fallback keeps the CLI byte-identical), agent/tool/task telemetry streams over **SSE** via global `crewai_event_bus` listeners attributed by a `ContextVar`, and a new `confirm_intake` tool adds an editable **intake-confirmation gate** before triage (a no-op at the terminal). The earlier stage-by-stage path (`pipeline.py` + the `/api/parse|triage|…` endpoints + `webapp/backend/test_parity.py`) is retained as a non-interactive, parity-tested test path. Full design + run instructions: [`README.md`](README.md).
