...
# ---------------------------------------------------------------------------
# Lay -> clinical chief-complaint synonym map.
#
# Applied ONLY to free-text *parser output* (the LLM's chief_complaints), via
# `clinicalize_complaint` below — NEVER to the tabular chief_complaint and NEVER
# at training time. That keeps the shared TF-IDF vectorizer and every trained
# model byte-identical (no retraining) and leaves the benchmark's tabular
# reference modes (`tool_direct`, `feature_vector`) uncontaminated, so the
# accuracy delta from enabling clinicalization cleanly isolates the map's value.
#
# Why a separate step (not folded into `normalize_complaint_text`): MIMIC-IV-ED
# triage staff already chart many semi-lay terms ("difficulty breathing",
# "dizzy", "lightheaded", "weak", "chest tightness", "head injury", ...), so
# rewriting them inside `normalize_complaint_text` would change the trained
# vocabulary and require retraining. Restricting the map to parsed free text
# means colliding terms (e.g. "lightheaded" -> "dizziness") simply redirect a
# parsed paraphrase onto whichever clinical feature the model already knows best.
#
# Targets are high-frequency clinical terms from the MIMIC-IV-ED triage
# `chiefcomplaint` vocabulary (so they align with the TF-IDF features the models
# were trained on). Keys are general lay paraphrases (consumer-health wording +
# the inverse of the Case Generation Agent's documented paraphrase rules) — NOT
# reverse-engineered from inspecting benchmark cases.
# ---------------------------------------------------------------------------
LAY_TO_CLINICAL = {
    # --- abdominal pain ---
