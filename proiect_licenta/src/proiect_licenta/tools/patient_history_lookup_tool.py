"""Patient history lookup tool (EHR simulation).

Simulates an EHR record lookup for a returning patient. Keyed on subject_id (not
fuzzy name/age matching), it returns the real 19-column PMH_FEATURE_COLS block
from the patient's prior MIMIC encounters, with the strict
prior_*time < current_intime leakage filter applied per query.

Asking the patient recovers the coarse pmh_<group> flags but not the recent-visit
numerics (days_since_last_admission, days_since_last_ed, n_prior_ed_visits,
same_complaint_as_prior) a patient can't report at the bedside; for a patient
already in the system this fetches them instead. Known patients get a real PMH
block; first-time/unknown subject_ids get known_patient=false and the runtime
keeps its ask-the-patient zero-fill path. The doctor tools consume pmh_block via
pmh_lookup_json (overriding the text-derived block when present).

It also returns a med_block: the reconciled home-med list (MED_FEATURE_COLS) from
the most recent prior ED stay (leakage-safe, < intime), consumed via
med_lookup_json. med_block is None for first-time/unknown patients or when no
prior stay carried a medrecon record. The current encounter can never leak
because its intime is not strictly before current_intime.
"""
