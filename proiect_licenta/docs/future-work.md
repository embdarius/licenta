...
**Vitals-conditioned centroids (DONE 2026-06-11).** Acting on the implication above: each candidate code gets a vitals centroid (mean standardized physiology = 6 z-scored vitals + 7 abnormal flags), compared to the patient's z-scored physiology by negative Euclidean distance, blended as a **third term** `w_text·text + w_vit·vitals + w_prev·prevalence` (simplex-tuned to text=0.4/vit=0.3/prev=0.3). This **raises the oracle ceiling itself** — out-of-sample @5: rollup 0.6698→0.6832 (+1.34pp), full code 0.4919→0.5129 (+2.10pp); E2E union v3-nurse rollup +1.34pp / full +1.94pp. Full code benefits more (more candidates to disambiguate physiologically); the term is near-neutral for normal-vitals patients. Snapshot vitals come from triage.csv so the builder stays fast; wired into `doctor_tool_v3` (3-term path when vitals present, 2-term fallback otherwise).

**Next work (deferred):**
- **Rhythm + longitudinal vitals in the centroid.** The vitals term uses snapshot triage vitals only. Adding cardiac rhythm + longitudinal min/max/last/delta (from `vitalsign.csv`) could extend the lift, at the cost of the slow vitalsign load in the builder.
