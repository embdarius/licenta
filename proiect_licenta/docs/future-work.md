...
**Vitals-conditioned centroids (DONE 2026-06-11).** Each candidate code gets a vitals centroid (mean standardized physiology), compared to the patient's z-scored physiology by negative Euclidean distance, blended as a **third term** `w_text·text + w_vit·vitals + w_prev·prevalence` (simplex-tuned). Built in two steps: **(1) snapshot** (6 vitals + 7 flags, 13-dim) lifted oracle@5 +1.34pp rollup / +2.10pp full; **(2) + cardiac rhythm (8 buckets + irregular) + per-vital trajectory deltas** from `vitalsign.csv` (28-dim total; min/max/last/counts excluded as snapshot-redundant) added another +0.81/+1.24pp, for **+2.15pp rollup / +3.34pp full @5** total (weights text=0.3/vit=0.4/prev=0.3 — vitals now the largest). E2E union v3-nurse: rollup +2.03pp / full +3.11pp. Full code benefits most (rhythm disambiguates the many specific cardiac codes). Builder cost rose to ~5–8 min (vitalsign load) but still skips the 3.3 GB PMH parse. Wired into `doctor_tool_v3` (3-term when vitals present, 2-term fallback; the standardizer's column list drives feature assembly).

**PMH-gated history term (DONE 2026-06-12).** Gated 4th term `w_text·text + w_vit·vitals + w_pmh·pmh + w_prev·prevalence` applied only to patients with prior history (`no_history==0`); the rest keep the 3-term vitals blend. PMH vector = 13 `pmh_<cat>` flags + `same_complaint_as_prior` (count/days numerics dropped to avoid the no-prior sentinel); centroids from has-history rows only; features from the vetted leakage-safe `aggregate_pmh` (`< intime`); the 3.3 GB discharge parse is cached to parquet. **64.9% of admitted patients have history.** Tuned has-PMH weights 0.2/0.4/0.2/0.2. Test-set lift on the has-PMH subset: oracle@5 rollup +0.97pp (0.7172→0.7269) / full +1.77pp (0.5657→0.5834); E2E union rollup +0.92pp / full +1.59pp. The **no-PMH subset is provably unchanged** (gate verified). Overall lift is diluted by the 35% no-PMH patients, so PMH is read on its subset. Wired into `doctor_tool_v3` (gated path when `no_history==0`). Cumulative Stage-2 oracle@5 rollup: text+prev 0.6698 → +vitals 0.6832 → +rhythm/delta 0.6913 → +PMH 0.6976 (0.7269 has-PMH).

**Next work (deferred):**
- **Past-medication rider.** Fold the `medrecon` medication-category flags into the gated PMH vector (meds reveal chronic conditions prior ICDs may miss). Noisier proxy for PMH; low marginal value, cheap once `medrecon` is loaded.
...
1e. ~~**Doctor disposition v3 inference wiring (plan section 3, Option B).**~~ **DONE (2026-05-30).** Full v3 swap landed in the same commit: `doctor_tool_v3_base.py` (NEW) for the pre-nurse initial assessment, `doctor_disposition_tool.py` (NEW) between nurse and reassessment, `doctor_tool_v3.py` (already on disk, now wired) for the post-nurse reassessment. The reassessment gates on the disposition task's REFINED `is_admitted` rather than triage's. `tasks.yaml` updated: triage hedges ESI top-2 when within 0.10, doctor assessment / reassessment surface top-3 + probabilities, disposition emits calibrated P(admit) / P(discharge) with calibration note. End-to-end smoke test passed. **Operating-point follow-up still queued:** the default 0.5 threshold trades sensitivity for specificity; threshold sweep on the calibration holdout (choose by under-triage-rate target or clinician utility) and asymmetric `scale_pos_weight` retrain are both documented next-steps. See [Inference status](agents/doctor-agent.md#inference-status--wired-2026-05-30) in the doctor-agent doc.
...
2b. ~~**Shrink the tabular→E2E NL-parse gap (lay↔clinical chief-complaint mismatch).**~~ **DONE (2026-06-20).** Two parser-side levers, both leaving the trained models frozen: a clinical-term parser prompt (Track 1, benchmark + live `tasks.yaml`) and a deterministic lay→clinical map `preprocessing.clinicalize_complaint` applied to parser output only (Track 2). On the frozen 250-case Flash headline each recovers ~4–6pp of the diagnosis@1 gap (prompt .417, map .399, vs .356 baseline / .472 ceiling — about half the gap); they are largely redundant, with the prompt best for diagnosis/ICD but slightly regressing department, and the map more balanced. (An earlier 150-case quick-read on the pre-completeness cohort showed larger, optimistic gains of ~+10–12pp.) **MedGemma-4B on the same cohort** confirms the safety-net hypothesis: weaker on plain parsing, but the clinical-term prompt lifts it more (diag@1 +8.0pp) and — unlike Flash — also *improves* department (+5.5pp vs Flash −2.5pp); Flash keeps an ESI-acuity edge, MedGemma edges department/ICD, diagnosis is a tie. Full per-target tables + audit JSONs in [`llm-backend.md` §5c](llm-backend.md). Paraphrase-robust **embeddings / training-time augmentation** remain the open follow-up (see entry 2 "tried and reverted" — that revert was on *exact* complaints, never under paraphrase).
...
### EHR / MRN lookup — demographics reconciliation (future work)
- **Age & gender are never reconciled against the record.** When a returning patient provides an MRN, `PatientHistoryLookupTool` returns only a `pmh_block` (PMH flags + prior-visit numerics) and a `med_block` (reconciled home meds) — these **override** the self-reported PMH / prior-admission count / medications in every tool that consumes the lookup (triage, disposition, v3). But `age` and `gender` are **not** fetched by the lookup at all; they always flow from the parsed intake (patient self-report). So if a patient misstates their age (says 52 when the record says 60), the system silently uses 52 — there is no record-age to prefer or cross-check against. This is a deliberate current limitation, not a bug: the EHR index stores prior-encounter history + meds, not demographics.
- **Proposed extension (small, mechanical).** Have `build_pmh_index` / `assemble_pmh_for_stay` also persist the subject's `anchor_age` + gender from `patients.csv`, return them in the lookup result (e.g. a `demographics` block), and either (a) prefer the record value over self-report for `age`/`gender` when `known_patient: true`, or (b) keep self-report but emit a reconciliation note (mirroring `self_report_not_in_record`) when they disagree, so the discrepancy is surfaced without silently changing the prediction. Option (b) matches the existing "record drives the features, self-report kept as an audit note" philosophy and is the safer default. Touch points: `patient_history_lookup_tool.py`, `pmh_features.py` (index assembly), and the `age`/`gender` argument resolution in `triage_tool.py` / `doctor_tool_v3*.py` / `doctor_disposition_tool.py`.
