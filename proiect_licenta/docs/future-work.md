...
**Next work (deferred):**
- **PMH / past-medication centroids (highest-value next).** Within a category, a patient's prior diagnoses (meds as a proxy) strongly predict the *exact recurrent* code — chronic exacerbations + return visits dominate admissions, and the `pmh_<cat>` flags come from strictly-prior ICDs (`< intime`, leakage-safe), so "last admission's diagnosis predicts this one's" is a legitimate, powerful signal. Caveats: (a) only available for patients with history → design as an **availability-gated** term (apply when `no_history==0`), report has-PMH vs no-PMH subsets separately; (b) needs the 3.3 GB discharge.csv parse → cache per-stay PMH features to keep the builder fast; (c) reuse `pmh_features.aggregate_pmh` verbatim for the leakage guard. Meds are a redundant proxy — optional rider, not standalone.
