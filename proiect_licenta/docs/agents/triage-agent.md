...
**Results (2026-06-27, 10 trials/head, held-out outer-test 83,617 rows; full tables + logs in [`docs/results/triage_hpo_group1/`](../results/triage_hpo_group1/)):** **reporting-only, not redeployed — the strongest "keep live" result in the program.** The multi-objective search **re-selected the incumbent as its deployed point on both heads**, and **no frontier config dominates the incumbent**.

- **Acuity:** the incumbent (boost 1.5/1.3/2.0) holds the **highest ESI-5 recall of the entire frontier (26.8%)** while staying competitive on exact accuracy (67.6%) and under-triage (15.6%). Every config that raised exact accuracy worsened *both* under-triage and ESI-5 recall (under-boosting the extreme classes — the exact trade `ESI_EXTREME_BOOST` refuses); every config that lowered under-triage gave up accuracy and/or ESI-5 recall. The boost vector is empirically validated as a well-placed point on the (exact, −under-triage, ESI-5 recall) trade-off.
- **Disposition:** ROC AUC is flat across the whole frontier (0.8607–0.8644) with the **incumbent at the top (0.8644)** — varying `scale_pos_weight`/`lr` does not improve discrimination; the rest of the frontier is just the decision threshold sliding along a fixed ROC curve. `scale_pos_weight` = raw ratio (exponent 1.0) confirmed.
- **Overtraining still bounded:** acuity trials that raised `n_estimators` to 8000 hit `best_iteration ≈ 7999` (mlogloss still falling) yet did not beat the incumbent — QWK plateaus by ~2,400 trees, so more trees buy nothing.
- **Optional future work (not pursued).** No redeploy candidate exists for triage; the only alternatives are operating-point shifts. The disposition frontier is a clean threshold operating-point curve (matters less than the doctor's 0.40 sweep, since the triage disposition is a baseline the doctor disposition later refines). Acuity trial 7 (under-triage −0.43pp at −0.37pp exact / −1.4pp ESI-5 recall) is the closest "more conservative" point but not a clear win. The Group-2 chaining re-run is moot (the Group-1 `selected` is the incumbent).
