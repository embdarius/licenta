...
- **Frozen Group-1.** Department: `lr=0.02`, `n_estimators=3000`, `early_stopping=100`, `multi:softprob`, sqrt-inverse class weighting (`class_weight_exponent=0.5`), 13-col diagnosis-softmax cascade (built once per run from a diagnosis model and reused across trials; the `report` stage uses the **live** `diagnosis_model.joblib` for the cascade in both arms so the comparison isolates the department Group-2). Note: the *search* cascade is trained with the tuned diagnosis params (`tuned_params.json` on Drive), whereas the live model — and the report cascade — is hand-picked (`tuned_params_applied: null`, same param family as v3_base); constant within the search, so it doesn't bias the ranking (see the search/report cascade caveat in [`docs/results/doctor_hpo/`](../results/doctor_hpo/)). Disposition: `lr=0.02`, `n_estimators=5000`, `early_stopping=150`, `binary:logistic`, `scale_pos_weight=sqrt(N_neg/N_pos)`, trained on the full 425K with the triage v3 soft cascade.
...
### Hyperparameter search — Group-1 (single-objective, reporting-only)

The doctor Group-1 sweep is now **implemented** as a sibling study in the same
engine + notebook (`--group group1`). Per the plan it is **single-objective** —
lower priority than triage's, mostly a cost/fidelity envelope — and keeps the
**same objectives + constraints** as the Group-2 study: department maximizes
macro-F1 (unconstrained); disposition maximizes ROC AUC subject to the under-triage
≤ incumbent constraint. Group-2 is frozen at `FROZEN_GROUP2`; the diagnosis head
stays out of scope (already swept). Everything lands in parallel `*_g1` files;
the live joblibs are never touched.

- **Engine:** [`scripts/tune_doctor_v3_heads.py`](../../scripts/tune_doctor_v3_heads.py) `--group group1`; **notebook:** [`notebooks/tune_doctor_hpo_v3.ipynb`](../../notebooks/tune_doctor_hpo_v3.ipynb) Section 5; **results:** [`docs/results/doctor_hpo_group1/`](../results/doctor_hpo_group1/).
...
- **Doctor Group-1 — disclose as a conditional-on-`lr` limitation, not a fix.** `lr`/`n_estimators` is a cost/fidelity envelope that plateaus under low-lr + early-stopping, and the class-weight exponent was already validated (~0.52) by the diagnosis sweep, so the expected payoff is low. The one real Group-1↔Group-2 coupling (`learning_rate × max_depth`) is only surfaced indirectly by the chaining re-run; a dedicated joint `lr × max_depth` study remains a possible follow-up.
- **Triage Group-1 — the deployed-point justification is the real work.** The multi-objective study (above) produces a Pareto frontier; choosing the deployed config from it needs a **clinical rationale, not just a metric delta** (re-opening the boost vector shifts the whole safety profile while improving a headline number). The `selected` block defaults to a knee heuristic (best headline subject to under-triage ≤ incumbent) and must be reviewed before any chaining re-run. Knob priority as originally scoped: `ESI_EXTREME_BOOST` (high) > disposition threshold (medium) > `lr × max_depth` (low–medium) > `neg_quadratic_kappa` metric (low, deliberately left out of the search).
