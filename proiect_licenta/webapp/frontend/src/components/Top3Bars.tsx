...
export default function Top3Bars({
  entries, color = "#38bdf8", showCode = false,
}: {
  entries: Top3Entry[];
  color?: string;
  showCode?: boolean;
        ...
        return (
          <li key={i}>
            <div className="mb-1 flex items-baseline justify-between gap-3 text-sm">
              <span className={i === 0 ? "font-semibold text-white" : "text-slate-300"}>
                {label}
                {code && code !== label && (
                  <span className="ml-1.5 text-xs text-slate-500">({code})</span>
                )}
              </span>
              <span className="tabular-nums text-slate-400">{p.toFixed(1)}%</span>
            </div>
